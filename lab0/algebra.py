...
aa = Sum([1,2, Sum([1, Sum([333]),3]),3])
bb = Product([1, 2, Sum([1]), Product([2,3])])
cc = Product([3, 4])
...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Product) and isinstance(expr2, Product):
        ...
        return Sum([multiply(expr1, tmp) for tmp in expr2])
    elif isinstance(expr1, Sum) and isinstance(expr2, Product):
        return do_multiply(expr2, expr1)
    else:
        return Sum([Product([tmp1] + [tmp2]) for tmp1 in expr1 for tmp2 in expr2])
