...
def show_result(testsummary, testcode, correct, got, expected, verbosity, fn_name_stephen,input_value_stephen):
    if correct:
        if verbosity > 0:
            print("%s: function %s Correct." % (testsummary, fn_name_stephen))
        if verbosity > 1:
            print('\t', testcode)
            print()
    else:
        ...
        print("")
        print("something wrong with: ", input_value_stephen)
        print("")
...
def run_test(test, lab):
    ...
    if type == 'VALUE':
        ...
        raise Error("Test Error: Unknown TYPE '%s'.  Please make sure you have downloaded the latest version of the tester script." %type)
...
def test_offline(verbosity=1):
    ...
    for index, (testname, getargs, testanswer, expected, fn_name) in enumerate(tests):
        ...
        show_result(summary, testname, correct, answer, expected, verbosity, fn_name, getargs())
